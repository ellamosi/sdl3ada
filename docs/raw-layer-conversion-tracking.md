...
| W1 Core raw support | Core utility raw families and value-package support types | `complete` | `SDL.Raw.Assert`, `SDL.Raw.Atomic`, `SDL.Raw.C_Pointers`, `SDL.Raw.CPUInfo`, `SDL.Raw.Error`, `SDL.Raw.Hints`, `SDL.Raw.Init`, `SDL.Raw.LoadSO`, `SDL.Raw.Log`, `SDL.Raw.Misc`, `SDL.Raw.Mutex`, `SDL.Raw.Platform`, `SDL.Raw.Power`, `SDL.Raw.Properties`, `SDL.Raw.System`, `SDL.Raw.Thread`, `SDL.Raw.Time`, `SDL.Raw.Timer`, and `SDL.Raw.Version` now exist, and the public cleanup for `SDL`, `SDL.Assertions`, `SDL.AsyncIO`, `SDL.Atomics`, `SDL.CPUS`, `SDL.Clipboard`, `SDL.Error`, `SDL.Filesystems`, `SDL.Hints`, `SDL.Libraries`, `SDL.Locale`, `SDL.Log`, `SDL.Misc`, `SDL.Mutexes`, `SDL.Platform`, `SDL.Power`, `SDL.Processes`, `SDL.Properties`, `SDL.Storage`, `SDL.Systems`, `SDL.Threads`, `SDL.Time`, `SDL.Timers`, and `SDL.Versions` is complete. Shared pure support types such as `SDL.Raw.Keyboard_Types` and `SDL.Raw.Video_Types` now cover the remaining value-layer scalar dependencies, so W1 no longer has open conversion blockers. |
| W2 Value package migration | Public value-heavy packages stop importing directly | `complete` | `SDL.Pens`, `SDL.Audio.Sample_Formats`, the root `SDL.Events` constants plus common/user event layouts, and `SDL.Events.Events` plus the keyboard, camera, sensor, touch, file-drop, window, mouse, pen, and joystick helper/layout packages now route through raw support families. The remaining handwritten pieces in `SDL.Events.Events`, `SDL.Events.Controllers`, and the SDL2-compatibility touch gesture carriers are now intentional compatibility surfaces rather than outstanding migration debt. |
| W3 Wrapper raw backfills | Missing raw families for audio, input, desktop, and device wrappers | `complete` | `SDL.Raw.Audio`, `SDL.Raw.Camera`, `SDL.Raw.Dialog`, `SDL.Raw.Gamepad`, `SDL.Raw.Haptic`, `SDL.Raw.HIDAPI`, `SDL.Raw.Joystick`, `SDL.Raw.Keyboard`, `SDL.Raw.MessageBox`, `SDL.Raw.Mouse`, `SDL.Raw.Pen`, `SDL.Raw.Sensor`, and `SDL.Raw.Tray` now exist. Pure support splits `SDL.Raw.Gamepad_Events` and `SDL.Raw.Joystick_Events` avoided the old pure-layer blocker, and the planned audio, device, desktop, and input wrappers plus joystick/gamepad maker helpers now route through raw. |
...
| `SDL.Raw.Gamepad_Events` | present | `complete` | Pure support split that owns the gamepad event-polling entry points used by `SDL.Events.Joysticks.Game_Controllers`; compatibility controller-event payloads remain intentionally frozen in `SDL.Events.Controllers` rather than growing new low-level behavior here. |
...
| `SDL.Raw.Keyboard` | present | `complete` | Raw keyboard now owns the typed keyboard focus, enumeration, modifier, state, naming/conversion, screen-keyboard, and text-input imports used by `SDL.Inputs.Keyboards`, while shared keyboard scalars live in `SDL.Raw.Keyboard_Types` and keyboard event payload layouts live in `SDL.Raw.Event_Layouts.Keyboards`. |
...
| `SDL.Raw.Touch` | present | `complete` | Added as a touch-support raw family and now owns touch identifiers, device-type and finger ABI structs, device enumeration, name/type queries, finger-list ABI, and cleanup helpers used by `SDL.Events.Touches`, while the native finger-event layout lives in `SDL.Raw.Event_Layouts.Touches`. |
...
| `SDL.Events.Joysticks.Game_Controllers` | public value layer | `complete` | Public package now routes gamepad event polling controls through pure `SDL.Raw.Gamepad_Events` and keeps only the controller axis/button compatibility names handwritten; the actual compatibility controller-event payload layouts remain intentionally frozen in `SDL.Events.Controllers`. |
