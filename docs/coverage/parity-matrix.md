...
| `SDL.Events.Cameras` | SDL3-native camera device event records and constants for added/removed/approved/denied notifications, wired into the `SDL.Events.Queue.Event` union view. | `device_smoke`, manual real-camera check |
...
| `SDL.Events.Pens` | SDL3-native pen proximity, touch, motion, button, and axis event records and constants, wired into the `SDL.Events.Queue.Event` union view without fabricating SDL2-era tablet abstractions. | `device_smoke`, manual real-pen check |
| `SDL.Events.Sensors` | SDL3-native sensor update event records and constants, wired into the `SDL.Events.Queue.Event` union view for sensor data notifications. | `device_smoke`, manual real-sensor check |
