...
| W2 Value package migration | Public value-heavy packages stop importing directly | `in progress` | Started with `SDL.Pens`, `SDL.Audio.Sample_Formats`, and `SDL.Events.Events` plus the keyboard and touch helper queries now route through raw support families; `SDL.Raw.Events` now owns opaque 128-byte event storage plus typed raw event-pointer callback ABI for queue/filter/window/description entry points, while the public event union layout remains handwritten above that raw storage and other pure helper/value units still need broader raw-backed cleanup. |
...
| Package | Target classification | Status | Notes |
| --- | --- | --- | --- |
| `SDL.Events` | public value layer | `complete` | Public root event package keeps event-type constants plus common/user event record layouts handwritten above raw event storage, with no direct SDL imports. |
