...
package body SDL.Hints is
   ...
   protected Callback_Registry is
      ...
      procedure Remove (Context : in out Callback_Context_Access);
   private
      Head : Callback_Context_Access := null;
   end Callback_Registry;

   protected body Callback_Registry is
      procedure Add (Context : in Callback_Context_Access) is
      begin
      ...
      begin
         while Current /= null loop
            exit when Current.Callback = Callback
              and then Current.User_Data = User_Data
              and then Current.Name /= null
              and then Current.Name.all = Name;
      ...
      procedure Remove (Context : in out Callback_Context_Access) is
         Previous : Callback_Context_Access := null;
         Current  : Callback_Context_Access := Head;
      begin
         while Current /= null loop
            exit when Current = Context;
            Previous := Current;
            Current := Current.Next;
         end loop;
   ...
   Allow_Topmost_Name                            : aliased constant String := "SDL_WINDOW_ALLOW_TOPMOST";
   ...
   Hint_Force_Raise_Window_Name                  : aliased constant String := "SDL_FORCE_RAISEWINDOW";
   ...
   IOS_Orientations_Name_Name                    : aliased constant String := "SDL_ORIENTATIONS";
   Hint_Orientations_Name                        : aliased constant String := "SDL_ORIENTATIONS";
   ...
   Vita_Touch_Mouse_Device_Name                  : aliased constant String := "SDL_VITA_TOUCH_MOUSE_DEVICE";
   ...
   Windows_Raw_Keyboard_Name                     : aliased constant String := "SDL_WINDOWS_RAW_KEYBOARD";
   ...
   Video_Driver_Name                             : aliased constant String := "SDL_VIDEO_DRIVER";
   Audio_Driver_Name                             : aliased constant String := "SDL_AUDIO_DRIVER";
   ...
   Hint_Name_Map : constant array (Hint'Range) of Hint_Names :=
     [Accelerometer_As_Joystick_Name'Access,
      ...
      Windows_Raw_Keyboard_Name'Access,
   ...
   begin
      raise Hint_Error with "Hint callback must not be null";
   end Raise_Invalid_Callback;

   procedure Hint_Trampoline
     (User_Data : in System.Address;
   ...
   begin
      ...
      Context.Callback
        (User_Data => Context.User_Data,
         ...
         Old_Value => String_Value (Old_Value),
   ...
   is
   begin
      return Get_Boolean (Value (Name), Default);
   end Get_Boolean;
   ...
   is
   begin
      Set (Hints.Value (Name), Value, Priority);
   end Set;
   ...
   begin
      ...
      then
         Callback_Registry.Remove (Context);
         return False;
      end if;
   ...
   is
   begin
      return Add_Callback (Value (Name), Callback, User_Data);
   end Add_Callback;
   ...
   begin
      ...
      Callback_Registry.Find (Name, Callback, User_Data, Context);
   ...
   begin
      Remove_Callback (Value (Name), Callback, User_Data);
   end Remove_Callback;
end SDL.Hints;
