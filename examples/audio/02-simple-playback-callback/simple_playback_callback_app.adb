...
package body Simple_Playback_Callback_App is
   ...
   procedure Fill_Sine
     (Current_Sample : in out Natural;
      ...
      Count          : in Positive);
   procedure Feed_The_Audio_Stream_More
   ...
   is
      ...
      App       : constant State_Access := To_State (User_Data);
      Remaining : Natural := 0;
   begin
      if App = null or else Additional_Amount <= 0 then
         return;
      end if;

      Remaining :=
        Natural (Additional_Amount) / SDL.Audio.Frame_Size (Tone_Spec);

      while Remaining > 0 loop
         declare
            Samples : Float_Sample_Array (1 .. 128);
            Count   : constant Positive :=
              Positive'Min (Positive (Remaining), Samples'Length);
         begin
            Fill_Sine (App.Current_Sine_Sample, Samples, Count);
            SDL.Audio.Streams.Put
              (Self        => Audio_Stream,
               Data        => Samples'Address,
               Byte_Length => Count * SDL.Audio.Frame_Size (Tone_Spec));
            Remaining := Remaining - Count;
         end;
      end loop;
   end Feed_The_Audio_Stream_More;
   ...
   begin
      ...
      SDL.Video.Renderers.Makers.Create
        (Window   => App.Window,
         Rend     => App.Renderer,
         Title    => "examples/audio/simple-playback-callback",
      ...
      SDL.Audio.Streams.Open
        (Self          => App.Stream,
         ...
         Callback      => Feed_The_Audio_Stream_More'Access,
         User_Data     => To_Address (App));
      ...
      if Exit_Code /= 0 then
         ...
         begin
            ...
            raise Program_Error with
              "simple_playback_callback exited with status"
...
end Simple_Playback_Callback_App;
