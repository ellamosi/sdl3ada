...
| `SDL.Raw.GPU` | present | `partial` | It no longer depends on public rectangle or pixel-format value packages, the GPU swapchain/window entry points now use raw video window handles, GPU scissor imports now use raw rectangle ABI types, fence waits now use typed raw fence arrays, render/compute pass plus binding-list imports now use raw-owned struct and handle pointer types instead of `System.Address`, and graphics-pipeline list fields now use typed raw access types for vertex buffer, vertex attribute, and colour target descriptions, but it still needs broader normalization to become a strict raw mirror. |
...
| `SDL.Raw.Video` | present | `partial` | Added as a starter family for video-driver enumeration, current-driver lookup, system-theme query, screensaver control, display enumeration/mode/bounds/orientation queries, GL/EGL attribute/context/proc/swap/library helpers, custom blend-mode composition, and broad window creation/query/mutation/surface/grab/progress/hit-test support used by `SDL.Video`, `SDL.Video.Displays`, `SDL.Video.GL`, maker helpers, window management, and `SDL.Video.Windows`; display point/rect and bounds queries now use raw-owned rectangle ABI types, window/GL/surface handle plus fullscreen-mode imports now use raw-owned pointer and mode types, window safe-area/update-rect/mouse-rect imports now route through `SDL.Raw.Rect`, and hit-test callbacks now reuse raw window and point ABI types, but broader generated-shape review is still pending. |
