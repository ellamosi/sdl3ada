...
| W5 Closure and enforcement | Lint/checking, compatibility freeze, final doc cleanup | `in progress` | The non-raw import baseline check now exists under `tools/`, the generated-raw ownership plus compatibility-freeze policy are documented, the compatibility queue is now explicitly closed for `SDL.C_Pointers`, `SDL.Events.Controllers`, `SDL.Inputs`, and `SDL.RWops*`, `SDL.Raw.C_Pointers` now owns the shared opaque SDL handle types used below raw, and handwritten video bridge glue now uses child support packages instead of linker-visible Ada hooks; remaining closure work is the raw-package dependency cleanup in `SDL.Raw.Main*`, plus the final normalization pass. |
