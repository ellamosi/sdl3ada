...
| W2 Value package migration | Public value-heavy packages stop importing directly | `in progress` | Started with `SDL.Pens`, and `SDL.Events.Keyboards` helper queries now route through raw keyboard support; event payload and other pure helper/value units still need broader raw-backed cleanup. |
