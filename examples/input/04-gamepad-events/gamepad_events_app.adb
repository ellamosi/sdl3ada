...
package body Gamepad_Events_App is
   ...
   use type SDL.Events.Controllers.Buttons;
   ...
   Max_Open_Gamepads     : constant Positive := 16;
   ...
   type Gamepad_Access is access SDL.Inputs.Joysticks.Game_Controllers.Game_Controller;

   type Gamepad_Slot is record
      ID     : SDL.Events.Joysticks.IDs := 0;
      Device : Gamepad_Access := null;
   end record;

   type Gamepad_Slot_List is
     array (Positive range 1 .. Max_Open_Gamepads) of Gamepad_Slot;
   ...
   procedure Free_Gamepad is new Ada.Unchecked_Deallocation
     (SDL.Inputs.Joysticks.Game_Controllers.Game_Controller, Gamepad_Access);
   ...
   procedure Close_Slot (Slot : in out Gamepad_Slot);
   ...
   function Axis_Name (Value : in SDL.Events.Controllers.Axes) return String;
   function Button_Name (Value : in SDL.Events.Controllers.Buttons) return String;
   procedure Find_Slot
     (App   : in State;
   ...
   procedure Close_Slot (Slot : in out Gamepad_Slot) is
   begin
      if Slot.Device /= null then
         SDL.Inputs.Joysticks.Game_Controllers.Close (Slot.Device.all);
         Free_Gamepad (Slot.Device);
      end if;
   ...
   begin
      for Index in App.Gamepads'Range loop
         Close_Slot (App.Gamepads (Index));
      end loop;
   ...
   function Axis_Name (Value : in SDL.Events.Controllers.Axes) return String is
   begin
      case Value is
         when SDL.Events.Controllers.Invalid =>
            return "";
         when SDL.Events.Controllers.Left_X =>
            return "leftx";
         when SDL.Events.Controllers.Left_Y =>
            return "lefty";
         when SDL.Events.Controllers.Right_X =>
            return "rightx";
         when SDL.Events.Controllers.Right_Y =>
            return "righty";
         when SDL.Events.Controllers.Left_Trigger =>
            return "lefttrigger";
         when SDL.Events.Controllers.Right_Trigger =>
            return "righttrigger";
      end case;
   end Axis_Name;

   function Button_Name (Value : in SDL.Events.Controllers.Buttons) return String is
   begin
      case Value is
         when SDL.Events.Controllers.Invalid =>
            return "";
         when SDL.Events.Controllers.A =>
            return "a";
         when SDL.Events.Controllers.B =>
            return "b";
         when SDL.Events.Controllers.X =>
            return "x";
         when SDL.Events.Controllers.Y =>
            return "y";
         when SDL.Events.Controllers.Back =>
            return "back";
         when SDL.Events.Controllers.Guide =>
            return "guide";
         when SDL.Events.Controllers.Start =>
            return "start";
         when SDL.Events.Controllers.Left_Stick =>
            return "leftstick";
         when SDL.Events.Controllers.Right_Stick =>
            return "rightstick";
         when SDL.Events.Controllers.Left_Shoulder =>
            return "leftshoulder";
         when SDL.Events.Controllers.Right_Shoulder =>
            return "rightshoulder";
         when SDL.Events.Controllers.Pad_Up =>
            return "dpup";
         when SDL.Events.Controllers.Pad_Down =>
            return "dpdown";
         when SDL.Events.Controllers.Pad_Left =>
            return "dpleft";
         when SDL.Events.Controllers.Pad_Right =>
            return "dpright";
         when SDL.Events.Controllers.Misc_1 =>
            return "misc1";
         when SDL.Events.Controllers.Right_Paddle_1 =>
            return "paddle1";
         when SDL.Events.Controllers.Left_Paddle_1 =>
            return "paddle2";
         when SDL.Events.Controllers.Right_Paddle_2 =>
            return "paddle3";
         when SDL.Events.Controllers.Left_Paddle_2 =>
            return "paddle4";
         when SDL.Events.Controllers.Touchpad =>
            return "touchpad";
         when SDL.Events.Controllers.Misc_2 =>
            return "misc2";
         when SDL.Events.Controllers.Misc_3 =>
            return "misc3";
         when SDL.Events.Controllers.Misc_4 =>
            return "misc4";
         when SDL.Events.Controllers.Misc_5 =>
            return "misc5";
         when SDL.Events.Controllers.Misc_6 =>
            return "misc6";
      end case;
   end Button_Name;
   ...
   begin
      for Candidate in App.Gamepads'Range loop
         if App.Gamepads (Candidate).Device /= null
           and then App.Gamepads (Candidate).ID = ID
         then
            Index := Candidate;
            return;
         end if;
   ...
   begin
      for Candidate in App.Gamepads'Range loop
         if App.Gamepads (Candidate).Device = null then
            Index := Candidate;
            return;
         end if;
      end loop;
   ...
   begin
      ...
      SDL.Video.Renderers.Makers.Create
        (Window   => App.Window,
         Rend     => App.Renderer,
         Title    => "examples/input/gamepad-events",
      ...
      Add_Message (App.all, 0, "Please plug in a gamepad.");
   ...
   begin
      if Event.Common.Event_Type = SDL.Events.Quit then
         ...
         begin
            ...
            if Slot_Index = 0 then
               Add_Message
                 (App.all,
                  Which,
                  "Gamepad #" & ID_Image (Which) & " add ignored: no free slots");
            else
               App.Gamepads (Slot_Index).Device :=
                 new SDL.Inputs.Joysticks.Game_Controllers.Game_Controller;

               begin
                  SDL.Inputs.Joysticks.Game_Controllers.Open
                    (App.Gamepads (Slot_Index).Device.all, Which);
                  App.Gamepads (Slot_Index).ID := Which;
                  ...
                  Add_Message
                    (App.all,
                     ...
                     & ID_Image (Which)
                     & " ('"
                     & SDL.Inputs.Joysticks.Game_Controllers.Get_Name
                         (App.Gamepads (Slot_Index).Device.all)
                     & "') added");
               ...
               exception
                  when Error : others =>
                     ...
                     Close_Slot (App.Gamepads (Slot_Index));
               end;
            end if;
         end;
         ...
         begin
            ...
            Add_Message
              (App.all, Which, "Gamepad #" & ID_Image (Which) & " removed");
         end;
      elsif Event.Common.Event_Type = SDL.Events.Controllers.Axis_Motion then
         declare
            Now : constant SDL.Timers.Milliseconds := SDL.Timers.Ticks;
         begin
            if Now >= App.Axis_Cooldown_Tick then
               ...
               Add_Message
                 (App.all,
                  Event.Controller_Axis.Which,
                  "Gamepad #"
                  & ID_Image (Event.Controller_Axis.Which)
                  & " axis "
                  & Axis_Name (Event.Controller_Axis.Axis)
                  & " -> "
                  & Int_Image (Integer (Event.Controller_Axis.Value)));
            end if;
         end;
      elsif Event.Common.Event_Type = SDL.Events.Controllers.Button_Up
        or else Event.Common.Event_Type = SDL.Events.Controllers.Button_Down
      then
         Add_Message
           (App.all,
            Event.Controller_Button.Which,
            "Gamepad #"
            & ID_Image (Event.Controller_Button.Which)
            & " button "
            & Button_Name (Event.Controller_Button.Button)
            & " -> "
            & (if SDL.Events.Controllers.Get_State (Event.Controller_Button) =
                   SDL.Events.Pressed
               then "PRESSED"
               else "RELEASED"));
         ...
         declare
            Slot_Index : Natural := 0;
         begin
            Find_Slot (App.all, Event.Joystick_Battery.Which, Slot_Index);
      ...
      if Exit_Code /= 0 then
         ...
         begin
            ...
            raise Program_Error with
              "gamepad_events exited with status"
