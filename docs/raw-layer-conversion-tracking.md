...
| W2 Value package migration | Public value-heavy packages stop importing directly | `in progress` | Started with `SDL.Pens`, `SDL.Audio.Sample_Formats`, the root `SDL.Events` constants, and `SDL.Events.Events` plus the keyboard and touch helper queries now route through raw support families; `SDL.Raw.Events` now owns opaque 128-byte event storage plus typed raw event-pointer callback ABI for queue/filter/window/description entry points, while the public event union layout remains handwritten above that raw storage and other pure helper/value units still need broader raw-backed cleanup. |
...
| Package | Target classification | Status | Notes |
| --- | --- | --- | --- |
| `SDL.Events` | public value layer | `complete` | Public root event package now routes root event-type constants through pure `SDL.Raw.Event_Types`, while common/user event record layouts stay handwritten above raw event storage. |
| `SDL.Events.Cameras` | public value layer | `complete` | Camera event constants and device-event layout remain handwritten above raw event storage, with no direct SDL imports. |
...
| `SDL.Events.Files` | public value layer | `complete` | Drop-event constants and file/drop record layout remain handwritten above raw event storage, with no direct SDL imports. |
...
| `SDL.Events.Mice` | public value layer | `complete` | Mouse event constants, button-mask helpers, and motion/button/wheel record layouts remain handwritten above raw event storage, with no direct SDL imports. |
| `SDL.Events.Pens` | public value layer | `complete` | Pen event constants and record layouts remain handwritten above raw event storage, while pen device/value helpers continue to route through `SDL.Pens` and `SDL.Raw.Pen` without direct SDL imports here. |
| `SDL.Events.Sensors` | public value layer | `complete` | Sensor update event constants and record layout remain handwritten above raw event storage, with no direct SDL imports. |
...
| `SDL.Events.Windows` | public value layer | `complete` | Window event constants, ID mapping helpers, and record layout remain handwritten above raw event storage, with no direct SDL imports. |
