...
| `SDL.Raw.GPU` | present | `partial` | It no longer depends on public rectangle or pixel-format value packages, the GPU swapchain/window entry points now use raw video window handles, and GPU scissor imports now use raw rectangle ABI types, but it still needs broader normalization to become a strict raw mirror. |
