...
| W2 Value package migration | Public value-heavy packages stop importing directly | `in progress` | Started with `SDL.Pens`, and `SDL.Events.Events` plus the keyboard and touch helper queries now route through raw support families; `SDL.Raw.Events` now owns opaque 128-byte event storage for queue/wait/push/window/description entry points, while event payload and other pure helper/value units still need broader raw-backed cleanup. |
...
| `SDL.Raw.Events` | present | `partial` | Added as a starter family for event queue, pump, filter/watch, enablement, registration, window lookup, and description entry points used by `SDL.Events.Events`; raw now owns opaque 128-byte event storage and typed event-pointer/window-handle imports for queue/wait/push/window/description paths, but the public event union layout and callback-visible event record still remain above raw for now. |
