...
| W4 Video/render/GPU | Video/render raw families, GPU normalization, public-type leak removal | `in progress` | `SDL.Raw.Metal`, `SDL.Raw.Pixels`, `SDL.Raw.Render`, and `SDL.Raw.Video` now exist as starter families for Metal view lifecycle, palette ownership/mutation, texture/renderer creation, and video/window driver, theme, screensaver, creation, and property queries. Window, texture, and renderer maker cleanup has started, but the broader video/render/GPU wrappers still need major raw backfills. |
...
| `SDL.Raw.Pixels` | present | `partial` | Added as a starter family for palette ABI ownership and mutation used by `SDL.Video.Palettes`; broader pixel-format support still needs normalization. |
...
| `SDL.Raw.Video` | present | `partial` | Added as a starter family for video-driver enumeration, current-driver lookup, system-theme query, custom blend-mode composition, screensaver control, window creation, property queries, and window ID lookup used by `SDL.Video`, maker helpers, and window management; the main display and window wrappers still need broad raw migration. |
