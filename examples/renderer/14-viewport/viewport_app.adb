...
package body Viewport_App is
   ...
   begin
      ...
      SDL.Video.Renderers.Makers.Create
        (Window   => App.Window,
         Rend     => App.Renderer,
         Title    => "examples/renderer/viewport",
   ...
   is
      ...
      Viewport : SDL.Video.Rectangles.Rectangle;
   begin
      SDL.Video.Renderers.Set_Draw_Colour (App.Renderer, 0.0, 0.0, 0.0, 1.0);
      SDL.Video.Renderers.Clear (App.Renderer);

      SDL.Video.Renderers.Reset_Viewport (App.Renderer);
      SDL.Video.Renderers.Copy_To (App.Renderer, App.Texture, Dst_Rect);

      Viewport :=
        (X      => SDL.Coordinate (Window_Width / 2),
         Y      => SDL.Coordinate (Window_Height / 2),
         Width  => SDL.Natural_Dimension (Window_Width / 2),
         Height => SDL.Natural_Dimension (Window_Height / 2));
      SDL.Video.Renderers.Set_Viewport (App.Renderer, Viewport);
      SDL.Video.Renderers.Copy_To (App.Renderer, App.Texture, Dst_Rect);

      Viewport :=
        (X      => 0,
         Y      => SDL.Coordinate (Window_Height - (Window_Height / 5)),
         Width  => SDL.Natural_Dimension (Window_Width / 5),
         Height => SDL.Natural_Dimension (Window_Height / 5));
      SDL.Video.Renderers.Set_Viewport (App.Renderer, Viewport);
      SDL.Video.Renderers.Copy_To (App.Renderer, App.Texture, Dst_Rect);
      ...
      Viewport :=
        (X      => 100,
         Y      => 200,
         Width  => SDL.Natural_Dimension (Window_Width),
         Height => SDL.Natural_Dimension (Window_Height));
      SDL.Video.Renderers.Set_Viewport (App.Renderer, Viewport);
      Dst_Rect.Y := -50.0;
      SDL.Video.Renderers.Copy_To (App.Renderer, App.Texture, Dst_Rect);
      ...
      if Exit_Code /= 0 then
         ...
         begin
            ...
            raise Program_Error with
              "viewport exited with status" & Integer'Image (Integer (Exit_Code));
         end;
      end if;
