...
| W2 Value package migration | Public value-heavy packages stop importing directly | `in progress` | Started with `SDL.Pens`, `SDL.Audio.Sample_Formats`, the root `SDL.Events` constants plus common/user event layouts, and `SDL.Events.Events` plus the keyboard, camera, sensor, and touch helper/layout packages now route through raw support families; `SDL.Raw.Events` now owns opaque 128-byte event storage plus typed raw event-pointer callback ABI for queue/filter/window/description entry points, while broader public event union and payload layout cleanup still remains. |
