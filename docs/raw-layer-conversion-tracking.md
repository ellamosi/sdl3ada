...
| W3 Wrapper raw backfills | Missing raw families for audio, input, desktop, and device wrappers | `in progress` | `SDL.Raw.Audio`, `SDL.Raw.Camera`, `SDL.Raw.Dialog`, `SDL.Raw.Gamepad`, `SDL.Raw.Haptic`, `SDL.Raw.HIDAPI`, `SDL.Raw.Joystick`, `SDL.Raw.Keyboard`, `SDL.Raw.MessageBox`, `SDL.Raw.Mouse`, `SDL.Raw.Pen`, `SDL.Raw.Sensor`, and `SDL.Raw.Tray` now exist. Pure support splits `SDL.Raw.Gamepad_Events` and `SDL.Raw.Joystick_Events` now carry event-polling entry points so the main raw families can expand without reviving a pure-layer blocker. `SDL.Audio`, `SDL.Audio.Devices`, `SDL.Audio.Streams`, `SDL.Cameras`, `SDL.Dialogs`, `SDL.HIDAPI`, `SDL.Haptics`, `SDL.Inputs.Joysticks`, `SDL.Inputs.Joysticks.Game_Controllers`, `SDL.Inputs.Keyboards`, `SDL.Inputs.Mice`, `SDL.Message_Boxes`, `SDL.Sensors`, `SDL.Trays`, joystick/gamepad event polling, and joystick/gamepad maker helpers now route through raw; the remaining W3 work is concentrated in cursor support and other still-unmigrated wrapper families. |
...
| `SDL.Raw.Keyboard` | present | `partial` | Added as a starter family for keyboard focus, enumeration, modifier, state, text-input, and screen-keyboard entry points used by `SDL.Inputs.Keyboards`; keyboard event payload and keycode/scancode helper migration still depends on `SDL.Raw.Events`. |
...
| `SDL.Raw.Mouse` | present | `partial` | Added as a starter family for mouse focus, enumeration, capture, state, relative-mode, transform, cursor-visibility, and warp entry points used by `SDL.Inputs.Mice`; cursor creation and broader event-helper migration remain separate work. |
...
| `SDL.Raw.Video` | present | `partial` | Added as a starter family for window creation, property queries, and window ID lookup; the main video, display, and window wrappers still need broad raw migration. |
...
| `SDL.Events.Keyboards` | public value layer | `not started` | `SDL.Raw.Keyboard` now exists for keyboard state and modifier support, but broader event layout and helper migration still needs `SDL.Raw.Events`. |
...
| `SDL.Inputs.Mice.Cursors` | public thick wrapper | `not started` | Mouse core support now exists under `SDL.Raw.Mouse`, but cursor creation and surface-backed helpers still need dedicated raw support, likely alongside `SDL.Raw.Surface`. |
