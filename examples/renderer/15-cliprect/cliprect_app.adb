...
package body Cliprect_App is
   ...
   Cliprect_Size  : constant SDL.Positive_Dimension := 250;
   Cliprect_Speed : constant Float := 200.0;

   type State is record
      Window            : SDL.Video.Windows.Window;
      Renderer          : SDL.Video.Renderers.Renderer;
      Texture           : SDL.Video.Textures.Texture;
      Cliprect_Position : SDL.Video.Rectangles.Float_Point :=
        (X => 0.0, Y => 0.0);
      Cliprect_Direction : SDL.Video.Rectangles.Float_Point :=
        (X => 1.0, Y => 1.0);
   ...
   function Round (Value : in Float) return SDL.Coordinate;
   ...
   function Round (Value : in Float) return SDL.Coordinate is
     (if Value >= 0.0
      then SDL.Coordinate (Integer (Value + 0.5))
      else SDL.Coordinate (Integer (Value - 0.5)));
   ...
   begin
      ...
      SDL.Video.Renderers.Makers.Create
        (Window   => App.Window,
         Rend     => App.Renderer,
         Title    => "examples/renderer/cliprect",
   ...
   is
      ...
      Distance : constant Float := Elapsed * Cliprect_Speed;
      Cliprect : SDL.Video.Rectangles.Rectangle;
   begin
      App.Cliprect_Position.X :=
        App.Cliprect_Position.X + (Distance * App.Cliprect_Direction.X);
      if App.Cliprect_Position.X < -Float (Cliprect_Size) then
         App.Cliprect_Position.X := -Float (Cliprect_Size);
         App.Cliprect_Direction.X := 1.0;
      elsif App.Cliprect_Position.X >= Float (Window_Width) then
         App.Cliprect_Position.X := Float (Window_Width - 1);
         App.Cliprect_Direction.X := -1.0;
      end if;

      App.Cliprect_Position.Y :=
        App.Cliprect_Position.Y + (Distance * App.Cliprect_Direction.Y);
      if App.Cliprect_Position.Y < -Float (Cliprect_Size) then
         App.Cliprect_Position.Y := -Float (Cliprect_Size);
         App.Cliprect_Direction.Y := 1.0;
      elsif App.Cliprect_Position.Y >= Float (Window_Height) then
         App.Cliprect_Position.Y := Float (Window_Height - 1);
         App.Cliprect_Direction.Y := -1.0;
      end if;

      Cliprect :=
        (X      => Round (App.Cliprect_Position.X),
         Y      => Round (App.Cliprect_Position.Y),
         Width  => Cliprect_Size,
         Height => Cliprect_Size);
      SDL.Video.Renderers.Set_Clip (App.Renderer, Cliprect);
      App.Last_Time := Now;
      ...
      if Exit_Code /= 0 then
         ...
         begin
            ...
            raise Program_Error with
              "cliprect exited with status" & Integer'Image (Integer (Exit_Code));
         end;
      end if;
