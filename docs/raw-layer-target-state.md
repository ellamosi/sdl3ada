...
| `SDL.Sensors`, `SDL.Haptics`, `SDL.Cameras`, `SDL.HIDAPI` | thick wrappers | keep `SDL.Raw.Sensor`; add `SDL.Raw.Haptic`, `SDL.Raw.Camera`; keep `SDL.Raw.HIDAPI` | Ownership, copying, and runtime checks stay handwritten. |
...
- Add missing raw families that already have public wrappers above them:
  ...
  `SDL.Raw.Events`, `SDL.Raw.Haptic`, `SDL.Raw.Keyboard`,
