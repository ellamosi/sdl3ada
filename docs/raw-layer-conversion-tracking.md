...
| `SDL.Raw.Rect` | present | `complete` | Raw rectangle support now owns the integer and floating rectangle/point ABI records plus the typed intersection, union, enclosing-points, and line-clipping imports used by `SDL.Video.Rectangles`; higher-level geometry helpers remain handwritten above raw. |
