...
| W2 Value package migration | Public value-heavy packages stop importing directly | `in progress` | Started with `SDL.Pens`, and `SDL.Events.Events` plus the keyboard and touch helper queries now route through raw support families; `SDL.Raw.Events` now owns opaque 128-byte event storage plus typed raw event-pointer callback ABI for queue/filter/window/description entry points, while the public event union layout remains handwritten above that raw storage and other pure helper/value units still need broader raw-backed cleanup. |
