...
package body Textures_App is
   ...
   begin
      ...
      SDL.Video.Renderers.Makers.Create
        (Window   => App.Window,
         Rend     => App.Renderer,
         Title    => "examples/renderer/textures",
      ...
      SDL.Video.Textures.Get_Size
        (App.Texture, App.Texture_Width, App.Texture_Height);
   ...
   is
      ...
      Dst_Rect : SDL.Video.Rectangles.Float_Rectangle :=
        (X      => 100.0 * Scale,
         Y      => 0.0,
         Width  => App.Texture_Width,
         Height => App.Texture_Height);
   begin
      ...
      Dst_Rect.X := (Float (Window_Width) - App.Texture_Width) / 2.0;
      Dst_Rect.Y := (Float (Window_Height) - App.Texture_Height) / 2.0;
      SDL.Video.Renderers.Copy_To (App.Renderer, App.Texture, Dst_Rect);

      Dst_Rect.X :=
        (Float (Window_Width) - App.Texture_Width) - (100.0 * Scale);
      Dst_Rect.Y := Float (Window_Height) - App.Texture_Height;
      SDL.Video.Renderers.Copy_To (App.Renderer, App.Texture, Dst_Rect);
      ...
      if Exit_Code /= 0 then
         ...
         begin
            ...
            raise Program_Error with
              "textures exited with status" & Integer'Image (Integer (Exit_Code));
         end;
      end if;
